...
def register_maintenance_api(app):
    ...
    # 9. Bulk upload maintenance logs (CSV parsed into MaintenanceLog list)
    @app.post("/maintenance/bulk-upload")
    def bulk_upload(logs: List[MaintenanceLog]):
        """
        Endpoint: POST /maintenance/bulk-upload
        Body: List of MaintenanceLog objects (parsed from CSV)
        Action: Insert multiple maintenance logs at once
