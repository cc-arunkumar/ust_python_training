...
def register_employee_api(app):
    ...
    # 9. Bulk upload employees (CSV parsed into EmployeeDirectory list)
    @app.post("/employees/bulk-upload")
    def bulk_upload(employees: List[EmployeeDirectory]):
        """
        Endpoint: POST /employees/bulk-upload
        Body: List of EmployeeDirectory objects (parsed from CSV)
        Action: Insert multiple employees at once
