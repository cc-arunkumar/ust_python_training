...
from schemas.task import ALLOWED_STATUSES
...
class TaskService:
    ...
    def create(db: Session, data: dict, current_user) -> Task:
        try:
            ...
            if data.get("assigned_to"):
                assignee = db.query(Employee).filter(Employee.emp_id == data["assigned_to"]).first()
                if not assignee:
                    raise HTTPException(404, f"Employee with ID {data['assigned_to']} not found")

            if data.get("reviewer"):
                reviewer = db.query(Employee).filter(Employee.emp_id == data["reviewer"]).first()
                if not reviewer:
    ...
    def update(db: Session, task: Task, data: dict, current_user) -> Task:
        try:
            ...
            if "reviewer" in data and data["reviewer"]:
                reviewer_emp = db.query(Employee).filter(Employee.emp_id == data["reviewer"]).first()
                if not reviewer_emp:
                    raise HTTPException(404, f"Reviewer with ID {data['reviewer']} not found")
    ...
    def update_status(db: Session, task: Task, status: str, current_user):
        try:
            if status not in ALLOWED_STATUSES:
                raise HTTPException(400, f"Invalid status. Must be one of: {', '.join(ALLOWED_STATUSES)}")
    ...
    @staticmethod
    def update_priority(db: Session, task: Task, priority: str, current_user):
        try:
