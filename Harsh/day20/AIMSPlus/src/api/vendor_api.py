...
def register_vendor_api(app):
    ...
    def read_vendor_by_status(active_status: str | None = None,current_user: User = Depends(get_current_user)):
        """
    ...
    # 8. Update full vendor record
    @app.put("/vendors/{vendor_id}")
    def update_vendor_endpoint(vendor_id: int, new_vendor: VendorMaster,current_user: User = Depends(get_current_user)):
        """
    ...
    # 9. Update only vendor status
    @app.patch("/vendors/{vendor_id}/status")   
    def update_status(vendor_id: int, new_status: str,current_user: User = Depends(get_current_user)):
        """
