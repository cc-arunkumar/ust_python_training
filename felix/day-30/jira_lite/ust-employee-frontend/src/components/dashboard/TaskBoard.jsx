...
const TaskBoard = ({ tasks, onStatusChange, onAddRemark, onAssign, userRole, employees }) => {
  ...
  const handleDrop = (e, newStatus) => {
    ...
    if (isValidTransition(draggedTask.status, newStatus, userRole, draggedTask)) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {Object.entries(tasksByStatus).map(([status, statusTasks]) => {
        ...
        const isValidDropZone =
          draggedTask && isValidTransition(draggedTask.status, status, userRole, draggedTask);
        ...
        return (
          <div
            key={status}
            className={`rounded-3xl border-2 ${config.borderColor} bg-gradient-to-br ${config.bgGradient} p-6 flex flex-col transition-all duration-300 shadow-lg hover:shadow-2xl relative overflow-hidden ${
              isValidDropZone
          ...
          >
            ...
            <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${config.gradient} opacity-10 rounded-full blur-3xl`}></div>
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${config.gradient} flex items-center justify-center shadow-lg text-white transform hover:scale-110 transition-transform duration-200`}>
                  {config.icon}
                </div>
                <div>
                  <h2 className={`font-bold text-lg ${config.textColor}`}>{status}</h2>
                  <p className="text-xs text-gray-500 font-medium">{sortedTasks.length} {sortedTasks.length === 1 ? 'task' : 'tasks'}</p>
                </div>
              </div>
              <div className={`px-4 py-2 rounded-xl bg-white shadow-md border-2 ${config.borderColor}`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center py-8">
                    <div className={`w-20 h-20 mx-auto mb-4 rounded-2xl ${config.iconBg} flex items-center justify-center`}>
                      <Circle size={40} className={`${config.textColor} opacity-30`} />
                    </div>
                    <p className="text-sm font-medium text-gray-400">No tasks yet</p>
                    <p className="text-xs text-gray-300 mt-1">Drag tasks here</p>
                  </div>
                </div>
              ) : (
