...
const EditUserModal = ({ token, user, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (updatePassword && (!formData.password || formData.password.length < 4)) {
    ...
    try {
      ...
      if (updatePassword && formData.password) {
        ...
        // Use existing password (you might need to handle this differently)
        updateData.password = user.password || 'unchanged';
      }

      await api.updateUser(token, user.emp_id, updateData);
      onSuccess();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border-2 border-gray-100">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-blue-500 to-purple-600 p-6 rounded-t-3xl border-b-2 border-purple-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-purple-100 text-sm mt-1">User ID: {user.emp_id}</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-all">
              ...
              />
              <div>
                <span className="font-bold text-gray-700">Update Password</span>
                <p className="text-xs text-gray-500 mt-0.5">Check to change user's password</p>
              </div>
            </label>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium pr-12"
                ...
                >
                  ...
                  <div className="flex-1">
                    <span className={`font-bold capitalize ${
                      formData.role.includes(role) ? 'text-blue-900' : 'text-gray-700'
                    }`}>
                      {role}
                    </span>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium"
            >
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:shadow-xl transition-all font-bold disabled:opacity-50 disabled:cursor-not-allowed"
            >
