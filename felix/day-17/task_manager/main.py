...
@app.post("/task")
def create_tasks(task: TaskModelCreate, current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        new_task = create_task(task_item,user)
        logg.insert_one((Log(
            username=user,
            action="CREATE",
            task_id=new_task.id
...
def get_task(task_id: int, current_user: User = Depends(get_current_user)):
    ...
    """
    try:
        tasks = get_task_by_id(task_id,current_user.username)
...
def update_task(task_id: int, task: TaskModelUpdate, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        details = update_task_by_id(task_id,task_item,current_user.username)
        logg.insert_one((Log(
...
def delete_task(task_id: int, current_user: User = Depends(get_current_user)):
    ...
    """
    try:
        msg = delete_task_by_id(task_id,current_user.username)
        logg.insert_one((Log(
        username=current_user.username,
        action="DELETE",
        task_id=task_id
