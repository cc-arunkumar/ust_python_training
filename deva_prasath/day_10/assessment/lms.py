# # lms.py  -- Menu-driven + Command-compatible Library CLI (Style C menu)
# import sys
# from library import Library
# from utils import safe_input, get_today_str, print_friendly_help
# from models import (
...
#     """
#     Returns (parts, raw_parts)
#     parts: command tokens with the first two tokens lowercased for routing
#     raw_parts: the original split tokens (preserve case for IDs)
#     """
...
#         return ["return", p[1]]  # preserve case for tx id
...
#         # allow "loans for U001" or "loans user U001"
#         idx = 2
#         if p[1] == "for":
#             idx = 2
...
# # -------------------------
# # Process a parsed command (shared by menu and typed commands)
# # -------------------------
# def process_command(parts: list, raw_parts: list, lib: Library):
#     """
#     parts: normalized/friendly parts used for routing
#     raw_parts: original tokens (preserve case for IDs and tx ids)
#     Returns True if processed (so caller can skip menu action); False if unknown.
#     """
#     if not parts:
#         return False
...
#             print(f"[✓] Book '{title}' added (ID: {book_id})")
#             return True

#         # update
#         if sub == "update":
#             if len(parts) < 3:
#                 # ask for id interactively
#                 bid = safe_input("Enter Book ID to update: ")
#                 bid = resolve_book_id(bid)
#             else:
#                 bid = resolve_book_id(parts[2])
...
#             print(f"[✓] Member '{name}' added (ID: {user_id})")
#             return True
...
#         # activate/deactivate/ban (use User methods directly)
#         if sub in ("activate", "deactivate", "ban"):
#             if len(parts) < 3:
#                 uid = safe_input(f"Enter Member ID to {sub}: ")
#                 uid = resolve_user_id(uid)
#             else:
#                 uid = resolve_user_id(parts[2])
#             user = lib.get_user(uid)
...
#     # ---------------------
#     if action == "borrow":
#         # allow interactive if missing args
#         if len(parts) >= 3:
#             uid = parts[1]
#             bid = parts[2]
#         else:
...
#     # ---------------------
#     if action == "return":
#         # preserve case for transaction id using raw_parts
#         if len(raw_parts) >= 2:
#             txid = raw_parts[1].strip()
#         else:
#             txid = safe_input("Enter Transaction ID to return: ")
#             txid = txid.strip()
...
#                 uid = safe_input("Enter Member ID: ")
#                 uid = resolve_user_id(uid)
#                 action = input("Type (activate/deactivate/ban): ").strip().lower()
#                 if action not in ("activate","deactivate","ban"):
#                     print("Invalid action.")
#                 else:
#                     parts, raw = normalize_command(f"user {action} {uid}")
#                     parts = friendly_normalize(parts)
#                     process_command(parts, raw, lib)
#             elif choice == "6":
#                 return
...
#     # allow direct command or interactive
#     uid = safe_input("Enter Member ID: ")
#     uid = resolve_user_id(uid)
#     bid = safe_input("Enter Book ID: ")
#     bid = resolve_book_id(bid)
...
# # -------------------------
# # Main loop (Style C menu + commands)
# # -------------------------
# def main():
#     lib = Library()
#     while True:
#         print_boxed_title("LIBRARY MAIN MENU")
...
#         print("7. Save")
#         print("8. Exit")
#         choice = input("Enter option number or type a command (type 'help' for commands): ").strip()
#         if not choice:
#             continue

#         # If numeric menu choice
#         if choice.isdigit():
#             if choice == "1":
#                 books_menu(lib)
#             elif choice == "2":
...
#         # Otherwise: treat input as typed command (compatibility mode)
#         try:
