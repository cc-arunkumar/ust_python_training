...
const RemarksChatModal = ({ taskId, isOpen, onClose }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      setSending(true);
      await remarksApi.postRemark(taskId, msg);
  ...
  const handleRetry = async (optimisticMsg) => {
    ...
    try {
      await remarksApi.postRemark(taskId, optimisticMsg.message);
      const serverMessages = await fetchMessages();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Task Remarks" size="medium">
      <div className="flex flex-col h-96">
        <div className="flex-1 overflow-auto p-3 space-y-3 bg-gray-50 border border-gray-200 flex flex-col">
          {messages.length === 0 && (
