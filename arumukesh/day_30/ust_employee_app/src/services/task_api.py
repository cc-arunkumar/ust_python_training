...
async def create_task(
    ...
    # Reviewer must be a manager
    reviewer = db.query(Employee).filter(Employee.emp_id == task.reviewer).first()
    if not reviewer or reviewer.mgr_id is None:
        raise HTTPException(status_code=400, detail="Reviewer must be a manager")
...
async def update_task_status(
    ...
    if not (
        current_user.emp_id == task.reviewer or
        current_user.emp_id == task.assigned_to
    ):
