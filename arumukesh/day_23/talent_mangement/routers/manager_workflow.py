...
async def get_manager_applications(current_user: dict, status_filter: list = None):
    ...
    if role == "TP Manager":
        ...
        query = {"job_rr_id": {"$in": job_rr_ids},"employee_id": {"$in": non_tp_emp_ids},"status": { "$nin": ["Draft","Allocated","Selected","Rejected","Withdrawn"] }}
        if status_filter:
...
@router.patch("/applications/{app_id}/shortlist")
...
@router.patch("/applications/{app_id}/interview")
...
@router.patch("/applications/{app_id}/select")
...
async def reject_candidate(app_id: str, current_user: dict = Depends(get_current_user)):
    app = await collections["applications"].find_one({"_id": app_id})
    if current_user["role"] != UserRole.WFM and app["status"] == ApplicationStatus.ALLOCATED and app["status"] == ApplicationStatus.SELECTED:
        raise HTTPException(403)
    await collections["applications"].update_one(
        {"_id": app_id},
...
@router.patch("/applications/{app_id}/allocate")
async def allocate(app_id: str, current_user: dict = Depends(get_current_user)):
    ...
    job = await collections["jobs"].find_one({"_id": app["job_rr_id"], "hm_id": current_user["employee_id"]})
    if not job:
        raise HTTPException(403)
    await collections["applications"].update_one(
        {"_id": app_id},
