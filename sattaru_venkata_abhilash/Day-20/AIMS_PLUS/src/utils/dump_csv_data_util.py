...
from config.db_connection import get_connection  # Importing the function to get a database connection


# SQL query for inserting asset data into the database
query = """
...
try:
    ...
    with open("valid_rows.csv", "r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            # Execute the insertion query for each asset in the CSV file
...
# SQL query for inserting new employee data into the employee_directory table
query = """
...
try:
    ...
    with open("valid_rows_emp.csv", "r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            # Execute the insert query for each row in the employee file
            cursor.execute(query, data)  # Execute the SQL insert for employee data
...
# SQL query for inserting maintenance log data into the maintenance_log table
query = """
...
try:
    conn = get_connection()  # Establish connection to the database
    cursor = conn.cursor()  # Create a cursor object for database interaction
    ...
    with open("valid_rows_maintain.csv", "r", encoding="utf-8") as file:
        ...
        # Iterate over each row in the CSV file
        for row in reader:
            # Check if required columns are present in the row
            if 'asset_tag' not in row or 'maintenance_type' not in row or 'vendor_name' not in row:
                ...
                continue  # Skip the row if required columns are missing

            # Prepare the data tuple for the SQL insert query
            data = (
            ...
            # Execute the insert query for the maintenance log data
            cursor.execute(query, data)
...
try:
    ...
    with open("valid_rows_vendor.csv", "r", encoding="utf-8") as file:
        ...
        # Iterate over each row in the CSV file
        for row in reader:
            # Prepare the data tuple for insertion into the vendor_master table
            data = (
                row["vendor_name"],       # vendor_name
            ...
            # Execute the SQL query to insert the vendor data into the database
            cursor.execute(query, data)
