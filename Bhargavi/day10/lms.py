# Main entry point for the LMS, providing an interactive menu to the user for managing books, users, and transactions.

# Orchestrates the program's flow, allowing the user to navigate between 
# different management sections like adding books, viewing users, etc.
...
def get_integer_input(prompt, default_value=None):
    """Helper function to get a valid integer input, with an optional default value."""
    while True:
...
def manage_users():
    ...
    while True:
        ...
        if choice == "1":
            ...
            max_loans = get_integer_input("Enter Max Loans (default 5): ", default_value=5)
            ...
            if users:
                print("\nALL USERS:")
                print("-" * 60)
                print(f"{'ID':<10} {'Name':<25} {'Email':<25} {'Status':<10}")
                print("-" * 60)
                for user in users:
                    name = user['name'][:22] if len(user['name']) > 22 else user['name']
                    email = user['email'][:22] if len(user['email']) > 22 else user['email']
                    status = user['status']
                    print(f"{user['user_id']:<10} {name:<25} {email:<25} {status:<10}")
            else:
