# Contains core classes and functions for managing books, users, loans, and transactions.

# Handles the logic for adding, removing, updating, and searching 
# for books and users, as well as managing borrowing and returning of books.
...
def add_transaction_record(tx_id, book_id, user_id, borrow_date, due_date, return_date="", status="borrowed"):
    ...
    tx_dict = {
        ...
        'borrow_date': str(borrow_date).strip(),
...
def update_transaction_record(tx_id, return_date, status="returned"):
    ...
    for tx in transactions:
        # be defensive: normalize values before compare
        existing_id = str(tx.get('tx_id', '')).strip()
        if existing_id == tx_id:
            tx['return_date'] = str(return_date).strip()
            tx['status'] = str(status).strip()
            updated = True
            break
...
# library.py (or your equivalent module)
def list_users():
    """Loads and returns the list of users with their loan details."""
    users = CSVStorage().load_users()  # Load the users from CSV storage

    for user in users:
        try:
            user['max_loans'] = int(user['max_loans'])
        except ValueError:
            print(f"Error converting 'max_loans' for user {user.get('user_id', 'Unknown')}")
            user['max_loans'] = 0

        try:
            user['active_loans'] = int(user.get('active_loans', 0))
        except ValueError:
            print(f"Error converting 'active_loans' for user {user.get('user_id', 'Unknown')}")
            user['active_loans'] = 0
