...
# LaptopAsset Pydantic Model
class LaptopAsset(BaseModel):
    asset_id: int = Field(..., ge=1, le=999999)  # asset_id must be between 1 and 999999
    serial_number: str = Field(..., pattern=r"^[A-Za-z0-9]{8,20}$")  # alphanumeric, 8-20 characters
    model_name: str = Field(..., min_length=3, max_length=50)  # model_name length between 3-50 characters
    assigned_to_emp_id: Optional[int] = Field(None, ge=1000, le=999999)  # employee ID (if assigned)
    status: str = Field(..., pattern=r"^(in_stock|assigned|retired)$")  # status must be one of these
    ...
    class Config:
        # Updated the config key to avoid the deprecation warning
        json_schema_extra = {
            "example": {
                "asset_id": 1,
                "serial_number": "ABC12345",
...
def create_laptop(laptop: LaptopAsset):
    ...
    if any(l for l in laptops if l.asset_id == laptop.asset_id):
...
# Test data (for convenience in testing)
test_data = [
    {
        "asset_id": 1,
        "serial_number": "ABC12345",
...
# Populate in-memory list with test data
for data in test_data:
    laptops.append(LaptopAsset(**data))


# #output

# 1. GET /laptops - Retrieve all laptop records
# Request:
# GET http://127.0.0.1:8000/laptops
# response:
# [
#     {
#         "asset_id": 1,
...
# 2. GET /laptops/{asset_id} - Retrieve a laptop by asset_id
# Request:
# GET http://127.0.0.1:8000/laptops/1
# Response:
# {
#     "asset_id": 1,
#     "serial_number": "ABC12345",
...
# 3. POST /laptops - Create a new laptop record
# Request:
# POST http://127.0.0.1:8000/laptops
# {
#     "asset_id": 4,
...
#  4. PUT /laptops/{asset_id} - Update an existing laptop record
# Request:
# PUT http://127.0.0.1:8000/laptops/2
# {
#     "asset_id": 2,
#     "serial_number": "XYZ98765",
#     "model_name": "HP EliteBook 840 G9",
...
# 5. DELETE /laptops/{asset_id} - Delete a laptop record
# Request:
# DELETE http://127.0.0.1:8000/laptops/3  
# Response:
# {
