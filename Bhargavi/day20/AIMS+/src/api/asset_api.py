...
def get_all_assets():
    try:
        # Establish database connection and fetch all assets
        connection = get_connection()
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM asset_inventory")
        assets = cursor.fetchall()  # Fetch all assets
        cursor.close()
        connection.close()
        return assets  # Return the list of assets
    except Exception as e:
        raise HTTPException(status_code=404, detail=str(e))  # Handle errors

# GET endpoint to filter assets by status
@asset_router.get("/list")
def get_assets_by_status(status: str):
    # Establish database connection and fetch assets by the given status
    connection = get_connection()
    cursor = connection.cursor()
    query = "SELECT * FROM asset_inventory WHERE asset_status = %s"
    cursor.execute(query, (status,))  # Execute query with status filter
    assets = cursor.fetchall()
    cursor.close()
    connection.close()
    return assets  # Return the filtered list of assets
...
def search_assets(column: str, keyword: str):
    ...
    return assets  # Return the search results

# GET endpoint to get asset by asset_id
@asset_router.get("/{id}")
def get_asset_by_id(id: int):
    # Fetch asset details by asset_id
    connection = get_connection()
    cursor = connection.cursor()
    cursor.execute("SELECT * FROM asset_inventory WHERE asset_id = %s", (id,))
    asset = cursor.fetchone()  # Fetch the asset
    cursor.close()
    connection.close()
    
    if asset is None:
        raise HTTPException(status_code=404, detail="Asset not found")  # Handle case where asset doesn't exist
...
def count_assets():
    ...
    count = cursor.fetchone()  # Fetch the total count
    cursor.close()
    connection.close()
    
    return {"total_assets": count[0]}  # Return the total number of assets
...
def create_asset(asset: Asset):
    try:
        ...
        cursor.close()
        connection.close()
        return {"message": "Asset created successfully"}  # Return success message
    except Exception as e:
...
def update_asset(id: int, asset: Asset):
    try:
        ...
        if count == 0:
            cursor.close()
            connection.close()
            raise HTTPException(status_code=404, detail="Asset not found")  # Handle non-existing asset
        ...
        return {"message": "Asset updated successfully"}  # Return success message
...
def update_asset_status(id: int, asset_status: str):
    try:
        # Update the asset status for the given asset_id
        connection = get_connection()
        cursor = connection.cursor()
...
def delete_asset(id: int):
    try:
        # Delete the asset with the given asset_id
        connection = get_connection()
        cursor = connection.cursor()
