...
def update_employee(emp_id,new_name,new_salary):
      try:
          conn = get_connection()
          cursor = conn.cursor()
          cursor.execute("UPDATE ust_db.emp SET EMP_NAME = %s, EMP_SALARY = %s WHERE EMP_ID = %s",
                         (new_name,new_salary,emp_id))
          conn.commit()
...
# read_all_employees()
# create_employee("VIRAT",50000)
# read_all_employees()
# update_employee(3,"VINU",31000.50)
# print("After Update:")
