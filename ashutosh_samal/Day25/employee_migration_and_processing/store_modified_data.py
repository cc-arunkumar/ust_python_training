from pymongo import MongoClient
from sql_read_and_modify import fetch_and_modify_employees 

# Function: Establish and return a connection to MongoDB 
def get_mongo_client():
    try:
        client = MongoClient("mongodb://localhost:27017/")  
        db = client["ust_mongo_db"]                        
        collection = db["employees"]                       
        return collection                                 
    except Exception as e:
        print(f"Error connecting to MongoDB: {str(e)}")    
        return None                                        


# Function: Store modified employee data into MongoDB
def store_in_mongodb(modified_employees):
    collection = get_mongo_client()  

    if collection is not None:
        collection.delete_many({})   # Clear existing documents in the collection

        if modified_employees:       # Check if modified employee data exists
            collection.insert_many(modified_employees)  
            print("Modified employee data inserted into MongoDB successfully!")
        else:
            print("No modified employee data to insert.")  
    else:
        print("Error: MongoDB connection failed.")         


# Fetch modified employees from MySQL and store them in MongoDB
modified_employees = fetch_and_modify_employees()


store_in_mongodb(modified_employees)
