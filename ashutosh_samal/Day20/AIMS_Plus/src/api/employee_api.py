...
from ..crud.employee_crud import (
    ...
    create_new_employee,
    modify_employee,
    modify_employee_status,
    remove_employee,
    find_employees_by_keyword,
...
def list_employees(status: Optional[str] = None,current_user: User = Depends(get_current_user)):
    try:
        employees = fetch_employees(status)  # Fetch employees based on status
        return employees
...
async def update_employee_endpoint(employee_id: int, employee: EmployeeCreate,current_user: User = Depends(get_current_user)):
    try:
...
async def update_employee_status_endpoint(employee_id: int, status: str,current_user: User = Depends(get_current_user)):
    try:
        modify_employee_status(employee_id, status)  # Update employee status
