...
class EmployeeBasic(BaseModel):
    ...
    official_email: str = Field(..., pattern=r"^[a-zA-Z0-9._%+-]+@ust\.com$",description="UST email address")
    department: str = Field(default="Telecom", description="Department name")
    location: str = Field(default="Bengaluru", description="Office location")


class SimCard(BaseModel):
    sim_number: str = Field(..., pattern=r"^\d{10}$",description="10-digit mobile number")
    provider: str = Field(default="Jio", description="Service provider")
    is_esim: bool = Field(default=False, description="eSIM flag")
    activation_year: int = Field(..., ge=2020, le=2025,description="Year of activation")


class DataPlan(BaseModel):
    name: str = Field(..., min_length=3, max_length=50, description="Plan name")
    monthly_gb: int = Field(..., gt=0, le=1000, description="Data volume in GB")
    speed_mbps: int = Field(default=50, ge=1, le=1000,description="Speed in Mbps")
    is_roaming_included: bool = Field(default=False, description="Roaming included flag")
