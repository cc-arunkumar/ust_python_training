...
def get_emp_by_id(emp_id : int):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("Select * from ust_db.employees where employee_id=%s",(emp_id,))
...
def update_emp(emp_id:int,emp:Employee):
    try:
        ...
        if get_emp_by_id(emp_id):
            query="""
    update ust_db.employees set first_name=%s,last_name=%s,email=%s,position=%s,salary=%s,hire_date=%s
    where employee_id=%s;
    """
