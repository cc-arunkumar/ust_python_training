...
app = FastAPI(title="Expert Level Validation")
...
class EmployeeTask(BaseModel):
    ...
    def check_hrs(self):
        ...
        total_subtask_hours = sum(x.hours_spent for x in subtask)
        if self.hours_spent < total_subtask_hours:
            raise HTTPException(status_code=422,
                detail=f"hours_spent ({self.hours_spent}) must be >= sum of subtasks ({total_subtask_hours})"
            )
        for data in self.subtasks:
            if(data.completed!=self.completed and self.completed==True):
                raise HTTPException(status_code=422,detail="cannot complete task with incomplete subtasks")
        for data in self.skills:
            if(data=="Python" and self.priority=="low"):
                raise HTTPException(status_code=422,detail="tasks with Python skill cannot have low priority")
        for data in self.attachments:
            if(data.file_name.endswith(".docx") and str(self.task_id) not in data.file_name):
        ...
        if(self.completed and self.priority=="high" and len(self.client_feedback.strip())<10):
        ...
        if not (fiscal_start <= self.start_date <= fiscal_end and fiscal_start <= self.end_date <= fiscal_end):
            raise HTTPException(status_code=422,detail="task dates must be within fiscal year")
        
        for sub in self.attachments:
            if sub.size_kb > 2000:
                raise HTTPException(status_code=422,detail="subtask attachment size cannot exceed 2000 KB")

        return self


employee : List[EmployeeTask] = []

@app.post("/validate")
def add_fun(data:EmployeeTask):
