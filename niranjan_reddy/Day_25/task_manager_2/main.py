...
from mongodb_logger import mongodb_logs  
from utils import get_all_tasks, get_task_by_id, update_task, delete_task, create_task
...
# Protected route to get the current logged-in user's info
@app.get("/me")
...
def update_task_endpoint(task_id: int, updated_task: TaskPost, current_user: User = Depends(get_curr_user)):
    ...
    # Log the task update action in MongoDB
...
def delete_task_endpoint(task_id: int, current_user: User = Depends(get_curr_user)):
    deleted_data = delete_task(task_id, current_user.id)  # Delete the task for the current user
    
    if not deleted_data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found or delete failed")
    
    # Log the task deletion action in MongoDB
