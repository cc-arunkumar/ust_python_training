...
async def get_hm_employees(
    ...
    try:
        #getting job resource request for the hm_id
        job_rr = await resouce_request_col.find_one({"hm_id": hm_id})
        
        if not job_rr:
            logger.warning(f"No job records found for HM ID: {hm_id}.")
            raise HTTPException(status_code=404,detail= f"No job records found for HM ID: {hm_id}.")
       
        # extracting job resource request id
        job_rr_id = str(job_rr.get("resource_request_id"))
 
        #getting applications with 'Allocated' status for the job_rr_id
        ...
        if not allocated_apps:
            ...
            raise HTTPException(status_code=404,detail= "No applications found for this job in 'Allocated' status.")
 
        unique_employee_ids = set(int(app["employee_id"]) for app in allocated_apps)
 
        # Fetch employee data for the unique employee IDs
...
async def wfm_view(
    ...
    try:
        #getting job resource request for the wfm_id
        ...
        # extracting job resource request ids
        ...
        #getting applications for the job_rr_ids    
        ...
        # extracting employee ids from applications
        ...
        # Fetch employee data for the extracted employee IDs
...
async def get_employees_from_applications(current_user: Dict[str, Any] = Depends(role_guard("TP Manager"))):
    ...
    try:
        ...
        #getting applications for TP Manager
        ...
        # Fetch TP employees for the extracted employee IDs
...
async def get_my_resume(
    ...
    # Extract employee ID from the current user session
...
# Determine which file ID to use for resume retrieval
        file_id = None
...
# Upload resume for the logged-in employee
...
# Update employee record with resume info
