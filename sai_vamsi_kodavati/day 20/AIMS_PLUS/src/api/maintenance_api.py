...
from src.crud.maintenance_crud import get_all, get_by_id, insert_maintenance_log, update_maintenance_log_by_id, delete_maintenance_log, search_maintenance_log
from src.auth.demo_jwt.main import User, get_curr_user
...
def get_logs(status_filter: Optional[str] = "",current_user:User = Depends(get_curr_user)):
    try:
...
def search_logs(field_type: str, keyword: str,current_user:User = Depends(get_curr_user)):
    try:
...
def count_logs(current_user:User = Depends(get_curr_user)):
    try:
        data = get_all()
        if data is None:
...
def get_log_by_id(log_id: int,current_user:User = Depends(get_curr_user)):
    try:
...
def create_maintenance_log(new_log: MaintenanceLogModel,current_user:User = Depends(get_curr_user)):
    try:
...
def update_maintenance_log(log_id: int, updated_log: MaintenanceLogModel,current_user:User = Depends(get_curr_user)):
    try:
...
def delete_maintenance_log_by_id(log_id: int,current_user:User = Depends(get_curr_user)):
    try:
...
def update_status(log_id: int, status: str,current_user:User = Depends(get_curr_user)):
    try:
