...
class User(Base):
    ...
    def __repr__(self):
        return f"User(roll_no = {self.roll_no} , name = : {self.name} , email : {self.email})"
...
def get_user_by_id(user_roll_no:int):
    try:
        session=SessionLocal()
        user= session.query(User).filter(User.roll_no==user_roll_no).first()
        session.close()
    except Exception as e:
        print("Exception ",e)
        return None
        ...
        session.close()
    return user
print("Employee with of specific id : ",get_user_by_id(2))


def update_by_id(user_roll_no=int, new_email=str):
    session=SessionLocal()
    person=session.query(User).filter(User.roll_no==user_roll_no).first()
...
def delete_by_id(user_roll_no=int):
    session=SessionLocal()
    person=session.query(User).filter(User.roll_no==user_roll_no).first()
