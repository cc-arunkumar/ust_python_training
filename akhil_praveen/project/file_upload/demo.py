...
class ResourceRequest(BaseModel):
    ...
    rr_type: Literal["New Project", "Existing Project"] = Field(..., alias="RR Type")
    priority: Literal["P1", "P4"] = Field(..., alias="Priority")  # Seen P1 and P4 in data
    ust_role: str = Field(..., alias="UST - Role")
    city: str = Field(..., alias="City")
    state: Optional[str] = Field(None, alias="State")
    country: str = Field(..., alias="Country")
    alternate_location: Optional[str] = Field(None, alias="Altenate Location")  # Note: typo in report
    ...
    billing_frequency: Optional[Literal["H", "D", "M"]] = Field(None, alias="Billing Frequency")  # H=Hourly
    ...
    ust_role_description: str = Field(..., alias="UST Role Description")
    ...
    recruiter_id: Optional[int] = Field(None, alias="Recruiter ID")
    ...
    mte_status: Literal["approved", "default", "Cancelled"] = Field(..., alias="MTE Status")
    ...
    duration_before_cancellation: Optional[str] = Field(None, alias="Duration before Cancellation")
    ...
    def str_to_bool(cls, v):
        if isinstance(v, bool):
            return v
        if str(v).strip().upper() in ("TRUE", "YES", "1"):
            return True
    ...
    def split_skills(cls, v):
        if not v or v == "NA" or str(v).strip() == "":
            return []
    ...
    @field_validator(
        ...
        "last_updated_on", mode="before"
    )
    @classmethod
    def parse_excel_date(cls, v):
    ...
    def parse_activity_datetime(cls, v):
        if not v or str(v).upper() == "NA":
            return None
        # Example: "06 Jan 2025, 04:48 AM (PT)"
        if isinstance(v, str):
            # Clean and parse with dateutil or manual
    ...
    def validate_dates_order(self):
        if self.rr_start_date and self.rr_end_date:
            if self.rr_start_date > self.rr_end_date:
                raise ValueError("RR Start Date cannot be after RR End Date")
