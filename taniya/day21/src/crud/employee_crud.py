...
def get_employees_by_id(employee_id: int):
    try:
        conn = get_connection()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
...
def update_employees(employee_id: int, emp: Employee):
    try:
        conn = get_connection()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        sql = """
        UPDATE employees SET first_name=%s, last_name=%s,
        email=%s, position=%s, salary=%s, hire_date=%s WHERE employee_id=%s
        """
        values = (
            emp.first_name,
            emp.last_name,
