import mysql.connector
import pymysql
def get_connection():
    conn=pymysql.connect(
        host="localhost",
        user="root",
        password="pass@word1",
        database="ust_db"
    )
    return conn
def read_all_employees():
    conn = get_connection()
    cursor=conn.cursor()
    cursor.execute("SELECT * FROM EMP;")
    rows = cursor.fetchall()
    for emp in rows:
        print(f"ID:{emp[0]} | Name: {emp[1]} | SALARY: {emp[2]}")
        
def read_employee_by_id(emp_id,):
    try:
        conn = get_connection()
        cursor=conn.cursor()
        cursor.execute("SELECT * FROM EMP WHERE EMP_ID = %s",(emp_id))
        row=cursor.fetchone()
        
        if row:
            print("Read employee data by id:",end="")
            print(f"ID:{row[0]} | Name: {row[1]} | SALARY: {row[2]}")
            return row
        else:
            print("Employee record not found for: {emp_id}")
            return None
    except Error as e:
        print("Error: ",e)
    finally:
        if conn:
            cursor.close()
            conn.close()
        print("connection closed succesfully")
def create_employee(name,salary):
    try:
        conn = get_connection()
        cursor=conn.cursor()
        cursor.execute("INSERT INTO ust_db.emp (EMP_NAME,EMP_SALARY) VALUES (%s,%s)",(name,salary))
        conn.commit()
        print("Employees recorded inserted successfully !")
    except Exception as e:
        print("Error:",e)
    finally:
        if conn:
            cursor.close()
            conn.close()
        print("connection closed")

def update_employee_id(emp_id,new_name,new_salary):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("update ust_db.emp SET EMP_NAME=%s,EMP_SALARY=%s WHERE EMP_ID=%s",(new_name,new_salary,emp_id))
        conn.commit()
        
        print("employee record has been updated usccessfully")
    except Exception as e:
        print("exception: ",e)
    finally:
        if conn:
            cursor.close()
            conn.close()
        print("Connection closed succesfully")
def delete_employee_id(emp_id):
    try:
        conn=get_connection()
        cursor=conn.cursor()
        emp_to_be_deleted = read_employee_by_id(emp_id)
        if emp_to_be_deleted != None:
            cursor.execute("DELETE FROM ust_db.emp WHERE EMP_ID = %s", (emp_id,))
            conn.commit()
            print("employee id deleted successfully")
        else:
            print("Employee record not found")
    except Exception as e:
        print("Exception:",e)
    finally:
        if conn:
            cursor.close()
            conn.close()
        print("deleted")
    
read_all_employees() 
# read_employee_by_id(2)
# create_employee("taniyaaa",60000)
# read_all_employees()
# update_employee_id(4,"Taniya Singh",80000)
# read_employee_by_id(4)
delete_employee_id(2)
read_all_employees()
    
    
    
    
    
    
    
#print("connection established successfully from database ",conn)

# conn.close()