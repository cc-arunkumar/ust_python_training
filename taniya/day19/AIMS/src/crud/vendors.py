...
def get_all_data():
    ...
    try:
        ...
        sql = "SELECT * FROM ust_aims.vendor_master"  # SQL query to fetch all vendors
...
def get_data_by_status(active_status: str):
    ...
    try:
        ...
        sql = "SELECT * FROM ust_aims.vendor_master WHERE active_status=%s"
...
def get_data_by_id(id:int):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        sql = "select * from ust_aims.vendor_master where vendor_id=%s"
