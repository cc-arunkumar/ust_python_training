...
class Book:
    def __init__(self, book_id, title, authors, isbn, tags, total_copies, available_copies):
        ...
        Validates that IDs and counts are correct when creating a new book.
        """
        if not book_id:
...
class Transaction:
    def __init__(self, tx_id, book_id, user_id, borrow_date, due_date, return_date=None, status="borrowed"):
        ...
        Validates IDs and status when creating a new transaction.
        """
        if not tx_id:
    ...
    def to_dict(self):
        """Convert transaction object into a dictionary for saving to CSV."""
        return {
            "tx_id": self.tx_id,
            "book_id": self.book_id,
            "user_id": self.user_id,
    ...
    def mark_returned(self, return_date):
        ...
        Raises TransactionError if transaction is not currently borrowed.
        """
