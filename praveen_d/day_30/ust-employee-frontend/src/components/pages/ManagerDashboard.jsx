...
const ManagerDashboard = ({
  ...
  // Updated: Show ALL tasks for manager (to populate views; change back to original filter if needed)
  // Original: const managerTasks = tasks.filter((t) => String(t.assigned_by).trim().toUpperCase() === String(user?.emp_id).trim().toUpperCase());
  const managerTasks = tasks; // Temporarily show all for full visibility
  ...
  const hasManagerTasks = managerTasks.length > 0;
                ...
                >
                  {tasksByStatus[status].length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      No {STATUS_META[status].label.toLowerCase()} tasks yet. Create one to get started!
                    </div>
                  ) : (
                ...
                {hasManagerTasks ? (
                  ...
                  <tr>
                    <td colSpan="6" className="px-4 py-8 text-center text-gray-500">
                      No tasks assigned by you yet. Create your first task to get started!
                    </td>
                  </tr>
                )}
              </tbody>
