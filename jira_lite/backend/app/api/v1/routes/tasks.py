...
def patch_status(
    ...
    if "admin" in roles:
        raise HTTPException(
            status_code=403,
            detail="Admin cannot change task status"
        )
    ...
    if "developer" in roles:
        if task["assigned_to"] != emp_id:
            raise HTTPException(
                status_code=403,
                detail="Developer can update only assigned tasks"
            )

        if current_status != "IN_PROGRESS" or status != "REVIEW":
            raise HTTPException(
                status_code=400,
                detail="Developer can move status only from IN_PROGRESS to REVIEW"
            )
    ...
    if "manager" in roles:
        if task["created_by"] != emp_id:
            raise HTTPException(
                status_code=403,
                detail="Manager can update only tasks created by him"
            )
        ...
        if current_status not in allowed_transitions or \
           allowed_transitions[current_status] != status:
            raise HTTPException(
                status_code=400,
                detail="Invalid status transition for manager"
            )
...
# ---------------------------------------------------
# PATCH PRIORITY (MANAGER ONLY - OWN TASKS)
# ---------------------------------------------------
...
def patch_priority(
    ...
    if "admin" in roles:
        raise HTTPException(
            status_code=403,
            detail="Admin cannot update task priority"
        )

    if "developer" in roles:
        raise HTTPException(
            status_code=403,
            detail="Developer cannot update task priority"
        )

    if "manager" in roles and task["created_by"] != emp_id:
        raise HTTPException(
            status_code=403,
            detail="Manager can update priority only for tasks created by him"
        )
