...
# Connect to MongoDB server running locally on default port 27017
client = MongoClient('mongodb://localhost:27017/')
db = client["ust_db"]              # Select database "ust_db"
collection = db["employee"]        # Select collection "employee"

# Test connection and print available databases
try:
...
def insert_by_one():
    """Insert a single employee record into the collection based on emp_id."""
    try:
        emp_id = int(input("Enter the employee ID to insert: "))
        with open("out.json", "r") as file:
            content = json.load(file)  # Load employee data from JSON file
            # Find employee with matching emp_id
            employee = next((item for item in content if item['emp_id'] == emp_id), None)
            if employee:
                collection.insert_one(employee)  # Insert into MongoDB
...
def insert_many():
    """Insert all employees from JSON file into the collection."""
    try:
        with open("out.json", "r") as file:
            content = json.load(file)
            collection.insert_many(content)  # Bulk insert
...
def find_by_dept(department):
    ...
    try:
        # Validate department name (only alphabets allowed)
        ...
        if result.count() == 0:  # Deprecated in newer PyMongo, use collection.count_documents instead
...
def update_department(emp_id, new_department):
    """Update the department of an employee by emp_id."""
    try:
...
def add_skill(emp_id, skill):
    """Add a skill to an employee's skill list (avoids duplicates)."""
    try:
        result = collection.update_one(
            {"emp_id": emp_id},
...
def remove_skill(emp_id, skill):
    ...
    try:
        result = collection.update_one(
            {"emp_id": emp_id},
            {"$pull": {"skills": skill}}  # $pull removes matching value
        )
        if result.matched_count > 0:
...
def delete_field(emp_id, field):
    """Delete a specific field from an employee record."""
    try:
        result = collection.update_one(
            {"emp_id": emp_id},
            {"$unset": {field: ""}}  # $unset removes field
        )
        if result.matched_count > 0:
...
def find_missing_skills():
    """Find employees who do not have a 'skills' field."""
    try:
