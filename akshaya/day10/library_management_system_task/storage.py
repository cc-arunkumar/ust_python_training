...
import csv
import os
from akshaya.day10.library_management_system_task.models import Book, User, Transaction

# Use simple relative folder: ./data
DATA_DIR = "data"
...
class CSVStorage:
    ...
    def _file(self, filename, headers):
        """Ensure file exists with headers and return correct path."""
    ...
    def load_books(self):
        ...
        return self._load("books.csv", headers, lambda r: Book(
            r.get("book_id",""),
            r.get("title",""),
    ...
    def save_books(self, books):
        headers = ["book_id","title","authors","isbn","tags","total_copies","available_copies"]
        self._save("books.csv", headers, books)

    # ---------------- Users ----------------
    def load_users(self):
        headers = ["user_id","name","email","status","max_loans"]
        return self._load("users.csv", headers, lambda r: User(
            r.get("user_id",""),
            r.get("name",""),
            r.get("email",""),
            r.get("status","active"),
    ...
    def save_users(self, users):
        headers = ["user_id","name","email","status","max_loans"]
        self._save("users.csv", headers, users)

    # ---------------- Transactions ----------------
    def load_transactions(self):
        headers = ["tx_id","book_id","user_id","borrow_date","due_date","return_date","status"]
        return self._load("transactions.csv", headers, lambda r: Transaction(
            r.get("tx_id",""),
            r.get("book_id",""),
            r.get("user_id",""),
            r.get("borrow_date",""),
    ...
    def save_transactions(self, txs):
        headers = ["tx_id","book_id","user_id","borrow_date","due_date","return_date","status"]
        self._save("transactions.csv", headers, txs)
